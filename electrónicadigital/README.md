# Electrónica digital

## Puertas Lógicas: son componentes capaces de hacer operaciones secinllas de cero y unos

#### Puerta Lógica NOT:

- Función: La salida es inversa de la entrada asi que si entra 1 sale 0 y viceversa.

<img width="481" height="310" alt="image" src="https://github.com/user-attachments/assets/41c0e383-5dc7-4a61-a284-3bad91ec6269" />

#### Puerta Lógica AND

- Función: La salida es 1 y cuando las dos entradas son 1 si no es 0

  <img width="289" height="175" alt="image" src="https://github.com/user-attachments/assets/1cdcd53d-673d-422b-b3e3-ecdda7337a12" />

  ### Puerta lógica OR

  - Función: La salida es 1 cuando cualquiera de entradas sea un 1 si las dos salidas son 0 sera 0
 
    <img width="289" height="175" alt="image" src="https://github.com/user-attachments/assets/6786f0c7-0891-4d85-868f-f19ae8992395" />

  ### Puerta lógica XOR

  - Función: La salida es 1 cuando cualquiera de los dos sea 1 pero no puede ser capaz los dos a la vez si los dos dan de cifra 1 o 0 su salida sera 0
 
    <img width="310" height="163" alt="image" src="https://github.com/user-attachments/assets/1c1722d8-b5a2-4cab-8bae-deb77c8eed8d" />

  ### Puerta lógica NOR

  - Función: La salida es 1 si ninguna tiene 1 si alguna tiene 1 su resultado sera 0
 
  <img width="289" height="175" alt="image" src="https://github.com/user-attachments/assets/8624376a-6788-43d1-bde1-1e3f0266972b" />

  ### Puerta lógica NAND

  - Función: La salida es 0 cuando las dos entradas son 1 si solo hay 1 o ninguno es 1

  <img width="289" height="175" alt="image" src="https://github.com/user-attachments/assets/3c308921-800f-49e0-bb0e-6cd5dadb7c8d" />

  ### Disposición interna

  <img width="225" height="225" alt="image" src="https://github.com/user-attachments/assets/7aafa2b0-80ed-41ec-99fd-b160227cde8a" />

  ## Puerta lógica NOT:
  La puerta lógica NOT tiene en común la puerta interna 1, 7 y 14 comunes con todos las otras puertas logicas la disposición interna en funcionamiento en pocas palabras su funcion es pares entradas inpares salidas.

## Puerta lógica AND:
La puerta lógica AND como los demas tienen en común 1,7,¡ y 14 y son dos de entrada y 1 de salida.

## Puerta lógica NAND: 
La puerta lógica NAND aparte de los comunes pues solo debe de estar las dos entradas activadas o desactivada para que la salida de corriente.

## Puerta lógica OR:

La puerta lógica OR quitando sus comunes de lado una entrada y solo una de ellas debe de estar activada para que la salida de corriente

## Puerta lógica XOR:
La puerta lógica XOR sin tener en cuenta sus comunes


## Puerta lógica NOR:
La puerta lógica NOR sin recordar sus comunes tiene una disposicion inversa de entradas teniendolas al reves
  





